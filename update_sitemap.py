...
# Read the file
with open(r'D:\ai-carbon-system\frontend\src\views\SiteMap3D.vue', 'r', encoding='utf-8') as f:
    content = f.read()

# 1. Add echarts import after OrbitControls import
if 'import * as echarts' not in content:
...
old_state = "const threeContainer = ref(null)"
new_state = """const threeContainer = ref(null)
const trendChart = ref(null)
const carbonTrendData = ref(null)
const optimizationSuggestions = ref(null)"""
content = content.replace(old_state, new_state)

# 3. Add new helper functions before getReductionProgressColor
old_func = "// 获取减排潜力进度条颜色\nconst getReductionProgressColor"
new_func = """// 获取优化建议类型名称
...
const renderTrendChart = (data) => {
  nextTick(() => {
    ...
    const option = {
      ...
      series: [
        {
          name: '碳排放量(tCO2)', type: 'bar', data: emissions,
          itemStyle: { color: '#f56c6c' }
        },
        {
          name: '绿电比例(%)', type: 'line', yAxisIndex: 1, data: greenRatios,
          smooth: true, lineStyle: { color: '#67c23a', width: 3 },
...
# 4. Add template sections after carbon-stats div
old_template = '''      <!-- 数据统计 -->
      <div v-if="sceneReady && carbonData" class="carbon-stats">'''
new_template = '''      <!-- 数据统计 -->
      <div v-if="sceneReady && carbonData" class="carbon-stats">'''

# Add trend chart card and optimization card after carbon-stats
chart_card = '''
      <!-- 碳排放趋势图表 -->
...
# Find the closing </el-card> after carbon-stats and insert new cards
old_closing = '''      </div>
    </el-card>
...
# Write back
with open(r'D:\ai-carbon-system\frontend\src\views\SiteMap3D.vue', 'w', encoding='utf-8') as f:
    f.write(content)

print("Successfully updated SiteMap3D.vue")

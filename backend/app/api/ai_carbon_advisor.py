...
INTENT_PATTERNS = {
    ...
    "scope_knowledge": [
        r"scope\s*(1|2|3)|范围[一二三123一二三]",
        r"什么是.*scope|什么是.*范围",
        r"scope.*排放|范围.*排放.*是什么",
        r"直接排放|间接排放|其他间接排放|范围[一二三]",
        r"碳排放.*定义|什么是.*碳.*排放",
        r"scope\s*1.*直接|scope\s*2.*间接|scope\s*3.*价值链",
        r"范围一.*锅炉|范围二.*电力|范围三.*供应链",
        r"组织.*边界|运营.*边界"
    ]
}
...
2. xxx""",
        ...
        "scope_knowledge": """你是碳排放分类专家。你需要：
1. 清晰解释Scope 1/2/3的定义和区别
2. 给出各范围的具体示例
3. 解释核算方法和适用标准
4. 帮助企业理解哪些排放属于哪个范围
...
- 对中小企业场景给出成本效益分析"""
    ...
    try:
        ...
        import logging
        logging.warning(f"LLM调用失败 (intent={intent}): {e}")
        reply = _fallback_reply(user_message, intent)
...
- ISO 14067 产品碳足迹标准""",
        ...
        "scope_knowledge": """【碳排放范围定义】

**Scope 1 - 直接排放**
企业拥有或控制的排放源直接产生的温室气体排放。
典型示例：锅炉燃烧天然气、自有车辆燃油消耗、制冷剂泄漏、生产工艺过程排放等。

**Scope 2 - 间接排放（能源）**
企业外购电力、热力等能源产生的间接排放。
典型示例：外购电力、外购蒸汽/热水、区域供冷供暖等。

**Scope 3 - 其他间接排放**
企业价值链上下游产生的其他间接排放，共15个类别。
典型示例：原材料采购（上游）、员工通勤、物流运输、产品使用阶段（下游）、废弃物处理等。

**核算标准**
- ISO 14064-1：组织层面温室气体核算
- GHG Protocol：温室气体核算体系
- GB/T 32150：工业企业温室气体排放核算通则

> 提示：配置AI大模型后可获得更详细的定制化解释""",
